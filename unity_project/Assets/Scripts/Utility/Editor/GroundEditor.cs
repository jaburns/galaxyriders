...
public class GroundEditor : Editor
{
    ...
    void snapNodesToGrid ()
    {
        targ.transform.position = new Vector3 (
            Mathf.Round (targ.transform.position.x / GRID_SIZE) * GRID_SIZE,
            Mathf.Round (targ.transform.position.y / GRID_SIZE) * GRID_SIZE,
        ...
        for (int i = 0; i < nodes.Count; ++i) {
            nodes [i] = new Vector2 (
                Mathf.Round (nodes [i].x / GRID_SIZE) * GRID_SIZE,
                Mathf.Round (nodes [i].y / GRID_SIZE) * GRID_SIZE
            );
        }
    }

...
// Modified from http://wiki.unity3d.com/index.php/TransformInspector
...
public class TransformInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        if (GUILayout.Button("Move To Here")) {
            shouldMove = true;
        }
    }
    ...
    {
        if (shouldMove) {
            var targ = (Transform)target;
            Undo.RecordObject(targ, "Moved using 'Move to Here'");
            targ.position = Camera.current.transform.position.WithZ(targ.position.z);
            shouldMove = false;
        }
    }

...
const Level World::GLOBAL_LEVEL = Level::from_data({
...
static const BakedLevel baked_global_level = BakedLevel::from_level(World::GLOBAL_LEVEL);
...
void World::step(const SharedInputState& old_input, const SharedInputState& new_input)
{
    ...
    const auto collision = baked_global_level.move_and_collide_circle(player.position, player.velocity , RADIUS, 0.0f);
